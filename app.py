...
请生成 {batch_size} 个不同的角色。要求：
1. 名字必须完全不同
2. 同一玩家的3个选项中，种族和职业组合不能重复
3. 团队角色定位要互补（输出、坦克、治疗、控制等）
4. 每个角色都要有独特的背景故事和动机
...
{CHARACTER_TEMPLATE}"""
                    ...
                    response = openai.ChatCompletion.create(
                        model="deepseek-chat",
                        messages=[
                            {"role": "system", "content": SYSTEM_PROMPT},  # 使用全局的 SYSTEM_PROMPT
                            {"role": "user", "content": prompt}  # 使用上面定义的 prompt
                        ],
                        temperature=0.8,  # 增加创造性
                        max_tokens=2000
                    )
                    
                    characters = [char.strip() for char in response.choices[0].message.content.split('---分隔线---') if char.strip()]
...
def get_ai_response(message, room, user_id, is_dm_query=False, timeout=None):
    ...
    max_retries = API_CONFIG.get('max_retries', 5)  # 获取重试次数，默认为5
